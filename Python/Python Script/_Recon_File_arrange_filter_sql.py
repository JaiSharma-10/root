...
filtered_df_insert_date = df.loc[(df['INSERT_DATE_'] >= '2024-10-01') & (df['INSERT_DATE_'] < '2024-12-17')] #filtering

filtered_df_insert_date['INSERT_DATE_0']= pd.to_datetime(df['INSERT_DATE_']).dt.date  #convert datetime into date

filtered_df_post_date = df.loc[(df['post_DATE_'] >= '2024-10-01') & (df['post_DATE_'] < '2024-12-17')] #filtering
...
with pd.ExcelWriter('Recon_6_final_verison.xlsx' ,engine="openpyxl") as writer:
